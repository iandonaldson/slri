...
# $Id: prebind_regex.pl,v 1.3 2004/07/16 20:39:24 kzheng Exp $
#*******************************************************************************
...
# Command syntax: 
#       perl prebind_regex.pl   -i input_file 
#                               [-o outputfile]  
#                               [-l log_file] 
#                               [-m remote/local] 
#                               [-n number] 
...
# -------------------------------------------------------------------
print "\nconnect to mysql prebind database ...\n";
my $dsn_mysql = "DBI:mysql:prebind:marge:33306";
...
while(<POINT>){
	...
	my $Aword = ($englishwords=~/\s$A\s/i);
	my $Bword = ($englishwords=~/\s$B\s/i);
...
#    print RESULT  "$pmid\t$piscore\t$gia\t$gib\n";
    print RESULT  "$pmid\t$gia\t$nama\t$gib\t$namb\t$piscore\n";
    $numLineOut++;
    $prev_pmid = $pmid;
} #end pointfile
