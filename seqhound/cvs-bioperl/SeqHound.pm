# $Id: SeqHound.pm,v 1.4 2005/02/23 22:52:46 ryao Exp $
...
=head1 DESCRIPTION
 ...
 SeqHound is a database of biological sequences and structures.
 This script allows the retrieval of sequence objects (Bio::Seq)
 from the SeqHound database at the Blueprint Initiative.
...
sub get_request {
	...
	$self->throw("please specify the SeqHound function for query");
    	...
    	my $url = URI->new($HOSTBASE . $CGILOCATION . $funcname);
...
sub _get_Seq_from_gbff
{
	...
	my $lcontent;
	if (ref($gi) =~ /array/i){
		#since seqrem has time out issue, if a long query list passed in,
		#better to chop the list and process seqrem call several times
		my @copyArr = @$gi;
		my @tempArr;
