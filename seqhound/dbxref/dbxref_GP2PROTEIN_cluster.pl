#! /usr/bin.perl
# $Id: dbxref_GP2PROTEIN_cluster.pl,v 1.2 2005/04/14 15:17:55 rcavero Exp $
#--------------------------------------------------------------------------
...
# =================  CLUSTERING ================================
    ...
    {
       dbxref->displayMessage('dbxref_GP2PROTEIN_cluster');
    }
    if ($cluster eq 'T')
    {
       if ( (! $offset) and (! $count2parse) )
       {
          displayMessage('dbxref_GP2PROTEIN_cluster');
       }
    }
# =============================================================
...
#    print RESULT "Parse file: $infile == \nTotal lines: $total\n";
    ...
    while (<FP>)
    {
	    ...
	    my @rowid = dbxref->selectID($sourcedb_id, $db_object_id, 0);
            if (!@rowid) # new record, create a top level id first
            {
                # parent_id =0
	        if(dbxref->executeInsert($sourcedb_id, $db_object_id, 0, 0, 'Col 1', 0) == 1)
		{
		   $countSPTR++;
		}
                #get id for above new record
                my @rowid2 = dbxref->selectID($sourcedb_id, $db_object_id, 0);
	    ...
	    if (dbxref->executeInsert($SPTR, $SPTR_id , $parent_id, 0, 'Col 2', 0 ) == 1)
	    {
	       $countOrg++;
	    }
	}	
...
# $Log: dbxref_GP2PROTEIN_cluster.pl,v $
# Revision 1.2  2005/04/14 15:17:55  rcavero
# Fix comments to reflect dbxref_GP2PROTEIN_cluster.pl
#
