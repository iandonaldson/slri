/* $Id: intrez_cb.c,v 1.16 2004/05/31 20:36:17 hlieu Exp $
*******************************************************************************************
...
DATA4
     ...
     *pd4Vastdb,
     *pd4Nrbioseq,
     *pd4Cddb; /*Coding region database*/
...
static FIELD4INFO f4iChrom[ ] ={
    ...
    { "ACCESS",  'C', 20,  0 },     /* accession for DNA unit */
    { "NAME",   'M', 10,  0 },     /* taxon and DNA unit name */
    { 0,0,0,0 }
};
...
Boolean LIBCALL OpenPUBSEQ(CharPtr name, Boolean Create)
{
   ...
   pf4psPMID = d4field(pd4Pubseq, "PMID");
   tagpsGi = d4tag(pd4Pubseq, "GI");
   tagpsMuid = d4tag(pd4Pubseq, "MUID");
   tagpsPMID = d4tag(pd4Pubseq, "PMID");
   MemFree(path);
...
Boolean LIBCALL ReindexREDUND(void)
{
	...
	if (pd4Redund==NULL)
		return FALSE;
	index=i4open(pd4Redund,NULL);
	if (index==NULL)
		return FALSE;	
	if (d4reindex(pd4Redund)!=0)
		return FALSE;
   	tagrdGi = d4tag(pd4Redund, "GI");
   	tagrdOrdinal = d4tag(pd4Redund, "ORDINAL");
   	tagrdGroup = d4tag(pd4Redund, "GROUP");
   ...
   for (rc = d4top(pd4Pubseq); rc == r4success; rc = d4skip(pd4Pubseq, (long) 1))
     {
       ...
       pmid = f4long(pf4psPMID);
       printf("%ld\t%ld%ld\n", (long) gival, (long) muid, (long) pmid);
     }
   printf("Number of records: %ld\n", (long) ginum);
   return TRUE;
}
   ...
   for (rc = d4top(pd4Chrom); rc == r4success; rc = d4skip(pd4Chrom, (long) 1))
     {
       ...
       u4ncpy(access, f4str(pf4chAccess), sizeof(access));
       pname = GetNameFromCHROM();
       printf("%ld\t%ld\t%ld%s%s\n", (long) taxval, (long) klval, (long) chromval, IsNullStr(TrimSpace(access)), IsNullStr(TrimSpace(pname)));
       pname = MemFree(pname);
     }
   printf("Number of records: %ld\n", (long) ginum);
   return TRUE;
...
Boolean LIBCALL AppendRecordPUBSEQ(StPubseqPtr pps, Int2 choice)
{
   ...
   /* need to search for the gi field see if an existing 
    * record is already in place. Start from the top. */
   d4tagSelect(pd4Pubseq, tagpsGi);
   d4top(pd4Pubseq);

   /* find the gi */
   for(rc = d4seekDouble(pd4Pubseq, (double) pps->gi); TRUE; rc = d4seekNextDouble(pd4Pubseq, (double)pps->gi))
   {	   
      if(rc != r4success)
      {
         /* printf("New record GI: %d %s%d\n", pps->gi, choice ? "PMID:" : "MUID:", pps->muid); */
         d4appendStart(pd4Pubseq, 0);
         d4blank(pd4Pubseq);
         /* gi not found, so new record */
         if(pps->gi)f4assignLong(pf4psGi, (long) pps->gi);

         /* store the pmid/muid field */
         if(choice == 0 && pps->muid)
            f4assignLong(pf4psMuid, (long) pps->muid);
         else if(choice == 1 && pps->muid)
            f4assignLong(pf4psPMID, (long) pps->muid);
         d4append(pd4Pubseq);
	 break;
      }else if( choice == 0 && f4int(pf4psMuid) == 0 ) {
         /* found the gi, and muid not filled in */
         /* printf("Adding MUID: old muid: %d Record: GI:%d %s%d\n", f4int(pf4psMuid), pps->gi, choice ? "PMID:" : "MUID:", pps->muid); */
         f4assignLong(pf4psMuid, (long) pps->muid);
	 break;
      } 
      else if(choice == 1 && f4int(pf4psPMID) == 0 ) {
         /* found gi, pmid not filled in */
         /* printf("Adding PMID: old pmid: %d Record: GI: %d %s%d\n", f4int(pf4psPMID), pps->gi, choice ? "PMID:" : "MUID:", pps->muid); */
         f4assignLong(pf4psPMID, (long) pps->muid);
	 break;
      }
   }
...
Boolean LIBCALL AppendRecordCHROM(StChromPtr pch)
{
   ...
   if(pch->access) f4assign(pf4chAccess, pch->access);
   if (pch->name != NULL)
    {

        rtn = f4memoAssignN(pf4chName, pch->name, (unsigned) strlen(pch->name)+1);
        if (rtn < 0)
         {
            ErrPostEx(SEV_ERROR, 0, 0, "AppendRecordCHROM: f4memoAssign failed.");
            return FALSE;
         }
    }
   d4append(pd4Chrom);
...
Boolean LIBCALL DeleteGIinPARTI(Int4 gi, Boolean destroy)
{
   ...
   d4top(pd4Partition);
   rc = d4seekDouble(pd4Partition, (double)gi);
   if (rc == r4success)
    {
      ...
      for(rc = d4seekNextDouble(pd4Partition, (double)gi); rc == r4success; rc = d4seekNextDouble(pd4Partition, (double)gi))
        {
          u4ncpy(div, f4str(pf4paParti), sizeof(div));
          if (destroy)
           {
...
Boolean LIBCALL DeleteGIinASNDB(CharPtr div, Int4 gi, Boolean destroy)
{
    ...
    if (!OpenASNDB(div, FALSE))
     {
       /* SK: Missed filename, can corrupt return address */
       ErrPostEx(SEV_ERROR,0,0, "Cannot open %s.", IsNullStr(div));
       return FALSE;
     }
    d4tagSelect(pd4Asndb, tagasGi);
    d4top(pd4Asndb);
    rc = d4seekDouble(pd4Asndb, (double)gi);
    if (rc == r4success)
     {
       if (destroy)
        {
       ...
       for(rc = d4seekNextDouble(pd4Asndb, (double)gi); rc == r4success; rc = d4seekNextDouble(pd4Asndb, (double)gi))
        {
...
Boolean LIBCALL DeleteGIinACCDB(Int4 gi, Boolean destroy)
{
   ...
   d4top(pd4Accdb);
   rc = d4seekDouble(pd4Accdb, (double)gi);
   if (rc == r4success)
   {
      ...
      for(rc = d4seekNextDouble(pd4Accdb, (double)gi); rc == r4success; rc = d4seekNextDouble(pd4Accdb, (double) gi))
        {
...
Boolean LIBCALL DeleteGIinTAXGI(Int4 gi, Boolean destroy)
{
   ...
   d4top(pd4Taxgi);
   rc = d4seekDouble(pd4Taxgi, (double) gi);
   if (rc == r4success)
    {
      if(f4long(pf4tgKloodge) != 0)
         ErrPostEx(SEV_ERROR,0,0, "DeleteGIinTAXGI: Removing kloodge info at %ld.", gi);
      if (destroy)
       {
         d4blank(pd4Taxgi);
      ...
      for(rc = d4seekNextDouble(pd4Taxgi, (double) gi); rc == r4success; rc = d4seekNextDouble(pd4Taxgi, (double) gi))
        {
          if(f4long(pf4tgKloodge) != 0)
             ErrPostEx(SEV_ERROR,0,0, "DeleteGIinTAXGI: Removing kloodge info at %ld.", gi);
          if (destroy)
...
Boolean LIBCALL DeleteGIinPUBSEQ(Int4 gi, Boolean destroy)
{
   ...
   d4tagSelect(pd4Pubseq, tagpsGi);
   d4top(pd4Pubseq);
   rc = d4seekDouble(pd4Pubseq, (double)gi);
   if (rc == r4success)
    {
      ...
      for(rc = d4seekNextDouble(pd4Pubseq, (double)gi); rc == r4success; rc = d4seekNextDouble(pd4Pubseq, (double) gi))
        {
...
Boolean LIBCALL DeleteGIinNUCPROT(Int4 gi, Boolean destroy)
{
   ...
   d4top(pd4Nucprot);
   rc = d4seekDouble(pd4Nucprot, (double)gi);
   if (rc == r4success)
    {
      ...
      for(rc = d4seekNextDouble(pd4Nucprot, (double)gi); rc == r4success; rc = d4seekNextDouble(pd4Nucprot, (double) gi))
        {
...
Boolean LIBCALL DeleteGIinSENGI(Int4 gi, Boolean destroy)
{
   ...
   d4top(pd4Sengi);
   rc = d4seekDouble(pd4Sengi, (double) gi);
   if (rc == r4success)
    {
      ...
      for(rc = d4seekNextDouble(pd4Sengi, (double)gi); rc == r4success; rc = d4seekNextDouble(pd4Sengi, (double) gi))
        {
...
Boolean LIBCALL DeleteGIinSENDB(CharPtr div, Int4 seid, Boolean destroy)
{
   ...
   d4top(pd4Sendb);
   rc = d4seekDouble(pd4Sendb, (double) seid);
   if (rc == r4success)
    {
      ...
      for(rc = d4seekNextDouble(pd4Sendb, (double) seid); rc == r4success; rc = d4seekNextDouble(pd4Sendb, (double) seid))
        {
...
Boolean LIBCALL DeleteGIinREDUND(Int4 gi, Boolean destroy)
{
   ...
   d4top(pd4Redund);
   rc = d4seekDouble(pd4Redund, (double)gi);
   if (rc == r4success)
    {
      ...
      for(rc = d4seekNextDouble(pd4Redund, (double)gi); rc == r4success; rc = d4seekNextDouble(pd4Redund, (double) gi))
        {
...
Boolean LIBCALL DeleteGIinCDDB( Int4 gi, Boolean destroy)
{
	...
	d4top(pd4Cddb);
	rc = d4seekDouble(pd4Cddb, (double) gi);
	if ( rc== r4success)
	{
		if (destroy)
		{
...
Int2 LIBCALL SearchASNDB(StAsndbPtr PNTR ppas)
{
   ...
   d4tagSelect(pd4Asndb, tagasGi);
   d4top(pd4Asndb);
   rc = d4seekDouble(pd4Asndb, (double)keynum);
   if (rc == r4success)
   {
      ...
      for(rc = d4seekNextDouble(pd4Asndb, (double)keynum); rc == r4success; rc = d4seekNextDouble(pd4Asndb, (double)keynum))
        {
...
Int2 LIBCALL SearchPARTI (StPartiPtr PNTR pppa)
{
   ...
   d4tagSelect(pd4Partition, tagpaGi);
   d4top(pd4Partition);
   rc = d4seekDouble(pd4Partition, (double)keynum);
   if (rc == r4success)
    {
      ppa->gi = f4long(pf4paGi);
      ppa->partition = TrimSpace(StrSave(f4str(pf4paParti)));
      for(rc = d4seekNextDouble(pd4Partition, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Partition, (double)keynum))
        {
          ppa->next = StPartiNew();
          ppa = ppa->next;
          ppa->gi = f4long(pf4paGi);
...
Int2 LIBCALL SearchNUCPROT (StNucprotPtr PNTR ppnc)
{
   ...
   d4top(pd4Nucprot);
   rc = d4seekDouble(pd4Nucprot, (double)keynum);
   if (rc == r4success)
    {
      pnc->gin = f4long(pf4ncGin);
      pnc->gia = f4long(pf4ncGia);
      for(rc = d4seekNextDouble(pd4Nucprot, (double)keynum); rc == r4success; rc = d4seekNextDouble(pd4Nucprot, (double)keynum))
        {
   ...
   if (pac->gi > 0)
    {
      ...
      d4tagSelect(pd4Accdb, tagacGi);
      d4top(pd4Accdb);
      rc = d4seekDouble(pd4Accdb, (double)keynum);
      if (rc == r4success)
       {
         ...
         for(rc = d4seekNextDouble(pd4Accdb, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Accdb, (double) keynum))
           {
             pac->next = StAccdbNew();
             pac = pac->next;
             pac->gi = f4long(pf4acGi);
...
CharPtr LIBCALL  GetTitlefromACCDB(void)
{
   ...
   /* EB - Changed to use f4memoPtr instead of f4memeStr.*/
   title = (CharPtr) f4memoPtr(pf4acTitle);
...
Int2  LIBCALL SearchPUBSEQ (StPubseqPtr PNTR ppps)
{
   ...
   if (pps->pmid > 0)
    {
      keynum = pps->pmid;
      d4tagSelect(pd4Pubseq, tagpsPMID);
    }
   if (keynum == 0)
    {
   ...
   d4top(pd4Pubseq);
   rc = d4seekDouble(pd4Pubseq, (double) keynum);
   if (rc == r4success)
    {
      pps->gi = f4long(pf4psGi);
      pps->muid = f4long(pf4psMuid);
      pps->pmid = f4long(pf4psPMID);
      for(rc = d4seekNextDouble(pd4Pubseq,(double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Pubseq, (double)keynum))
        {
...
ValNodePtr LIBCALL SearchTAXGIDNA(Int4 taxid)
{
   ...
   d4top(pd4Taxgi);
   rc = d4seekDouble(pd4Taxgi, (double)taxid);
   if (rc == r4success)
    {
      u4ncpy(type, f4str(pf4tgType), sizeof(type));

      if (strstr(type, "NA") != NULL)
       {
             gi = f4long(pf4tgGi);
             pvngi = Slri_ValNodeAddInt(&pvngi, 0, gi);
       }
      for(rc = d4seekNextDouble(pd4Taxgi, (double)taxid); rc == r4success; rc = d4seekNextDouble(pd4Taxgi, (double)taxid))
        {
          u4ncpy(type, f4str(pf4tgType), sizeof(type));
          if (strstr(type, "NA") != NULL)
           {
...
Int2 LIBCALL SearchTAXGI (StTaxgiPtr PNTR pptg)
{
   ...
   d4top(pd4Taxgi);
   rc = d4seekDouble(pd4Taxgi, (double)keynum);
   if (rc == r4success)
    {
      ...
      for(rc = d4seekNextDouble(pd4Taxgi, (double)keynum); rc == r4success; rc = d4seekNextDouble(pd4Taxgi, (double)keynum))
        {
          ptg->next = StTaxgiNew();
          ptg = ptg->next;
          ptg->gi = f4long(pf4tgGi);
...
CharPtr LIBCALL  GetNameFromCHROM(void)
{
   ...
   title = (CharPtr) f4memoStr(pf4chName);
   ptitle = StringSave(title);
   return ptitle;
}
...
Int2 LIBCALL SearchCHROM (StChromPtr PNTR ppch)
{
   ...
   pch = *ppch;
   if ((pch->chromfl <= 0) && (pch->kloodge <= 0) && (pch->taxid <= 0) && (pch->access == NULL))
    {
   ...
   {
      key0 = pch->access;
      d4tagSelect(pd4Chrom, tagchAcc);
      d4top(pd4Chrom);
      key = PadString(key0);
      rc = d4seekN(pd4Chrom, key,(int) strlen(key));
      if (rc == r4success)
      {
         pch->kloodge = f4long(pf4chKloodge);
         pch->taxid = f4long(pf4chTaxId);
         pch->chromfl = f4long(pf4chChromfl);
         pch->access = TrimSpace(StrSave(f4str(pf4chAccess)));
         pch->name = GetNameFromCHROM();
         for(rc = d4seekNextN(pd4Chrom, key, (int) strlen(key)); rc == r4success; rc = d4seekNextN(pd4Chrom, key, (int) strlen(key)))
         {
            pch->next = StChromNew();
            pch = pch->next;
            pch->kloodge = f4long(pf4chKloodge);
   ...
   d4top(pd4Chrom);
   rc = d4seekDouble(pd4Chrom,(double) keynum);
   if (rc == r4success)
    {
      pch->kloodge = f4long(pf4chKloodge);
      pch->taxid = f4long(pf4chTaxId);
      ...
      pch->access = TrimSpace(StrSave(f4str(pf4chAccess)));
      pch->name = GetNameFromCHROM();
      for(rc = d4seekNextDouble(pd4Chrom, (double)keynum); rc == r4success; rc = d4seekNextDouble(pd4Chrom, (double)keynum))
        {
          pch->next = StChromNew();
          pch = pch->next;
          pch->kloodge = f4long(pf4chKloodge);
...
static Boolean LIBCALL ExpandSeqEntry(SeqEntryPtr PNTR ppse)
{
   ...
   {
      ErrPostEx(SEV_ERROR,0,0,"ExpandSeqEntry: Null input parameter.");
      return FALSE;
   }
...
Int2 LIBCALL SearchSENDB(StSendbPtr PNTR ppsd)
{
   ...
   d4tagSelect(pd4Sendb, tagsdSeid);
   d4top(pd4Sendb);
   rc = d4seekDouble(pd4Sendb, (double) keynum);
   if (rc == r4success)
   {
      ...
      psd->asn1 = pse;
      for(rc = d4seekNextDouble(pd4Sendb, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Sendb, (double) keynum))
        {
...
Int2  LIBCALL SearchSENGI (StSengiPtr PNTR ppsg)
{
   ...
   d4top(pd4Sengi);
   rc = d4seekDouble(pd4Sengi, (double)keynum);
   if (rc == r4success)
    {
      psg->gi = f4long(pf4sgGi);
      psg->seid = f4long(pf4sgSeid);
      psg->division = TrimSpace(StrSave(f4str(pf4sgDivision)));
      for(rc = d4seekNextDouble(pd4Sengi, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Sengi, (double)keynum))
        {
          psg->next = StSengiNew();
          psg = psg->next;
          psg->gi = f4long(pf4sgGi);
...
Int2 LIBCALL SearchMMDBEx (StMmdbPtr PNTR ppmd,Int4 mdlLvl, Int4 maxModels)
{
   ...
   if (keynum > 0)
    {
      d4top(pd4Mmdb);
      rc = d4seekDouble(pd4Mmdb, (double) keynum);
      if (rc == r4success)
       {
         pmd->mmdbid = f4long(pf4mdMmdbId);
         pmd->asn1 = GetBiostrucfromMMDB(mdlLvl, maxModels);
         if (pmd->asn1 == NULL)
          {
         ...
         for(rc = d4seekNextDouble(pd4Mmdb, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Mmdb, (double) keynum))
           {
             pmd->next = StMmdbNew();
             pmd = pmd->next;
             pmd->mmdbid = f4long(pf4mdMmdbId);
...
Int2 LIBCALL SearchMMGI (StMmgiPtr PNTR ppmg)
{
   ...
   d4top(pd4Mmgi);
   rc = d4seekDouble(pd4Mmgi, (double) keynum);
   if (rc == r4success)
    {
      pmg->mmdbid = f4long(pf4mgMmdbId);
      pmg->gi = f4long(pf4mgGi);
      for(rc = d4seekNextDouble(pd4Mmgi, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Mmgi, (double) keynum))
       {
         pmg->next = StMmgiNew();
         pmg = pmg->next;
         pmg->mmdbid = f4long(pf4mgMmdbId);
...
Int2 LIBCALL SearchDOMDB (StDomdbPtr PNTR ppdd)
{
   ...
   if (keynum > 0)
    {
      d4top(pd4Domdb);
      rc = d4seekDouble(pd4Domdb, (double)keynum);
      if (rc == r4success)
       {
         ...
         for(rc = d4seekNextDouble(pd4Domdb, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Domdb, (double) keynum))
           {
             pdd->next = StDomdbNew();
             pdd = pdd->next;
             pdd->mmdbid = f4long(pf4ddMmdbId);
...
Int2 LIBCALL SearchREDUND (StRedundPtr PNTR pprd)
{
   ...
   d4top(pd4Redund);
   rc = d4seekDouble(pd4Redund,(double) keynum);
   if (rc == r4success)
    {
      prd->gi = f4long(pf4rdGi);
      prd->ordinal = f4long(pf4rdOrdinal);
      prd->group = f4long(pf4rdGroup);
      for(rc = d4seekNextDouble(pd4Redund, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Redund, (double) keynum))
       {
...
Int2 LIBCALL SearchCDDB(StCddbPtr PNTR ppcd)
{
   ...
   d4tagSelect(pd4Cddb, tagcdGi);
   d4top(pd4Cddb);
   rc = d4seekDouble(pd4Cddb, (double)keynum);
   if (rc == r4success)
   {
      /* fill out the StSendb structure */
      ...
      pcd->asn1 = sfp;
      for(rc = d4seekNextDouble(pd4Cddb, (double) keynum); rc == r4success; rc = d4seekNextDouble(pd4Cddb, (double)keynum))
        {
          /* add and fill out the next StSendb structure */
          pcd->next = StCddbNew();
          pcd = pcd->next;
...
Boolean LIBCALL EditRecordTAXGIByGI(StTaxgiPtr ptg, Boolean overwrite)
{
    ...
    if (rc == r4success)
     ...
     {
       ErrPostEx(SEV_INFO,0,0, "EditRecordTAXGIByGI: GI %ld not found in TAXGI database.", (long) ptg->gi);
       /* KM 030204 return FALSE if GI not found; makes it easier to track success from comgen */
       StTaxgiFree(ptgtemp);
       return FALSE;
     }
...
ValNodePtr LIBCALL SHoundAllGenAccess(void)
{
   ...
   for (rc = d4top(pd4Chrom); rc == r4success; rc = d4skip(pd4Chrom, (long) 1))
   {
        ptemp = TrimSpace(StrSave(f4str(pf4chAccess)));
        ValNodeAddStr(&pvnacc, 0, ptemp);
   }
   return pvnacc;
}
...
Int4 LIBCALL SHoundGetKloodgeFromAccess(CharPtr paccess)
{
   ...
   if(paccess == NULL)
   {
       ErrPostEx(SEV_ERROR,0,0, "SHoundGetKloodgeFromAccess invalid param.");
       return 0;
   }
   pch = StChromNew();
   pch->access = paccess;
   ...
   if (res == 0)
    {
      ErrPostEx(SEV_ERROR,0,0, "SHoundGetKloodgeFromAccess: SearchCHROM failed.");
      StChromFree(pch);
      return 0;
    }
   if (res == -1)
...
ValNodePtr LIBCALL SHoundAllPartition(CharPtr ppartition)
{
   ...
   for (rc = d4top(pd4Partition); rc == r4success; rc = d4skip(pd4Partition, (long) 1))
     {
       gival = f4long(pf4paGi);
       ptemp = TrimSpace(StrSave(f4str(pf4paParti)));
       if(strcmp(ptemp, ppartition) == 0)
...
void LIBCALL ResetStSENGI(StSengiPtr PNTR ppsg)
{
   ...
   /* SK: NULL ptr check */
   /* Bug 1337 fix, correct typo psg to ppsg */
   if(ppsg == NULL || *ppsg == NULL)
    {
   ...
   psg->seid = 0;
   psg->gi = 0;
   /* Bug 1337 related fix, free psg->division to prevent mem leak */
   MemFree (psg->division);
   psg->division = NULL;
...
static Boolean LIBCALL ExpandSeqEntryforTindex(SeqEntryPtr PNTR ppse, Int4 Gi)
{
   ...
   if (pse->choice == 1) /* It is a Bioseq */
   ...
   else if (pse->choice == 2) /* it is nested Bioseq-set */
    {
         ...
         while (pseNew != NULL)
            {
             if(!ExpandSeqEntryforTindex(&pseNew,Gi))
               {
                ErrPostEx(SEV_ERROR,0,0,"ExpandSeqEntry: The recursive call to ToBiseqAs failed");
                return FALSE;
               }
...
/*
 $Log: intrez_cb.c,v $
 Revision 1.16  2004/05/31 20:36:17  hlieu
 minor compiler warning fixes for bug 2185

 Revision 1.15  2004/05/03 19:53:08  eburgess
 Changed GetTitleFromACCDB() to use f4memPtr() instead of f4memoStr. Also
 fixed memory leak.

 Revision 1.14  2004/03/17 20:03:02  eburgess
 Victor: Added SetCodeBaseAutoIdxTrue() to set the codebase autoOpen flag. f

 Revision 1.13  2004/03/09 23:25:05  hfeldman
 changed reindexing of redund

 Revision 1.12  2004/03/09 21:17:22  hfeldman
 added functions for opening redun with no index

 Revision 1.11  2004/02/16 17:19:52  idonalds
 Changed return logic in EditRecordTAXGIByGI to support changes to comgen.c

 Revision 1.10  2004/01/22 20:35:28  vgu
 removing PUBSEQ #if 0 defines since it is now in production

 Revision 1.9  2004/01/21 21:35:01  vgu
 code change from fixing bug 1337

 Revision 1.8  2004/01/06 22:32:15  skuznets
 Code audit has been done sucessfully.
 Possible NULL pointer issues has been eliminated.

 Revision 1.7  2003/11/27 19:30:19  idonalds
 code4largeOn required to access files greater than 2GB

 Revision 1.6  2003/10/27 17:00:36  haocl
 commenting out PMID from pubseq for now

 Revision 1.5  2003/10/27 16:44:02  haocl
 for now commenting out PMID in PUBSEQ

 Revision 1.4  2003/10/24 17:53:11  feldman
 Added SHoundGet3DEx function to allow user to choose model level and num models when fetching biostruc

 Revision 1.3  2003/09/04 16:41:18  haocl
 commented out print statements

 Revision 1.2  2003/09/04 16:40:03  haocl
 fixed AppendRecordPUBSEQ so that it will support multiple records with same
 gi, but different PMID and MUID

 Revision 1.1.1.16  2003/07/26 08:42:02  root
 Automated import from Sourceforge.net

 Revision 1.18  2003/07/25 17:20:15  haocl
 fixed SearchPUBSEQ to retrieve pmid also

 Revision 1.17  2003/07/25 15:15:21  haocl
 altered PUBSEQ database to hold both Muid & Pmid.
 Altered source code to reflect database changes.

 Revision 1.16  2003/05/23 02:16:34  kaca
 fixed recursive call in ExpandSeqEntryforTindex

 Revision 1.15  2002/11/04 21:56:16  kaca
 added SHoundAllPartition

 Revision 1.14  2002/10/17 19:58:13  kaca
 added two fields to CHROM table

 Revision 1.13  2002/09/27 16:44:38  kaca
 added read only mode for SHound initialization

 Revision 1.12  2002/07/08 16:58:02  kaca
 renamed two RPS-BLAST functions Accession is DomainId

 Revision 1.11  2002/04/03 20:35:47  kaca
 removed RPSDB and DOMNAME related functions; they reside in rpsdb_cb.c

 Revision 1.10  2002/03/18 18:01:01  betel
 Fixed DOMNAME Append and Print calls

 Revision 1.9  2002/03/09 21:32:17  betel
 Added ResetStDOMNAME function

 Revision 1.8  2002/02/28 21:36:48  betel
 Fixed compiler warnings

 Revision 1.7  2002/02/28 17:23:00  micheld
 ...
 Revision 1.6  2002/02/26 22:51:54  betel
 Compile error fixes

 Revision 1.5  2002/02/21 22:07:15  betel
 Addition of RPS results and Cdd tables

 Revision 1.4  2002/02/19 20:17:18  kaca
 replaced the SearchTAXGIDNA/protein with a lighweight unabstracted version

 Revision 1.3  2002/02/10 19:54:20  mdumontier
 abstracted internals of SHoundProteinsFromTaxID and SHoundDNAFromTaxID to database layer

 Revision 1.2  2002/01/23 23:24:55  kaca
 changed SHoundIsInited function and moved it from intrez_cb.c to intrez.c

 Revision 1.1  2001/12/19 23:30:42  kaca
