/* $Id: ll_db.c,v 1.3 2004/05/13 17:38:19 hlieu Exp $
**************************************************************************
...
Int4 LL_SearchGODB(StGOdbPtr PNTR ppgo)
{
  ...
  if (rc == r4success)
  {
    ...
    pgo->eviCode = StringSave(f4str(pF4LL_EVICODE));
...
Int4 LL_SearchCDDDB(StCDDdbPtr PNTR ppcdd)
{
  ...
  if (rc == r4success){
    pcdd->npid = f4int(pF4LL_LL_CDD);
    /*pcdd->cddid = GetMemoField(pF4LL_CDD);*/
    pcdd->cddid = StringSave(f4str(pF4LL_CDD));
...
Int4 LL_SearchLLDB(StLLdbPtr PNTR ppll)
{
  ...
  if (rc == r4success)
  {
    pll->npid = f4int(pF4LL_LL_LLID);
    pll->llid = f4int(pF4LL_LLINK_ID);
...
Int4 LL_Append2OMIM_DB(Int4 NPid, Int4 OMIM)
{
  ...
  f4assignInt(pF4LL_LL_OMIM, NPid);
...
Int4 LL_Append2LLID_DB(Int4 NPid, Int4 LLid, CharPtr pMAP)
{
  ...
  f4assignInt(pF4LL_LL_LLID, NPid);
  f4assignInt(pF4LL_LLINK_ID, LLid);
...
/*
$Log: ll_db.c,v $
Revision 1.3  2004/05/13 17:38:19  hlieu
