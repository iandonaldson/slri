...
# .intrezrc has been
# If rebuild flag is TRUE, rebuild mmdb tables
if ($rebuild_flag eq "TRUE")
{
  #Remove the MMDB tables in the directory mmdb
  #PATH hard-coded!! Change path needed if deployed elsewhere
  my $mmdbdir="/hibbert-local/shoundtest/mmdb";
  system("rm ${mmdbdir}/*");
...
{
  print MAIL "cbmmdb tables up-to-date. \n";
}
...
close (MAIL);
$oldIdxSave = $oldIdxFileFull."_save";
system("mv $oldIdxFileFull $oldIdxSave");
