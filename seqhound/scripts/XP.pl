...
#
#  DESCRIPTION
#  	Experimental Sequences (XP) daily update
#
...
# Construct today's update file names
$name = $project.$today;
$htmlfile = $path.$name.".html";
$rawfile = $path.$name.".raw";
$txtfile = $path.$name.".txt";
$tmpfile = $path.$name."_tmp.txt";
$logfile = $fulllog."/".$project.".log";

# Open log and title it
open(LOG, ">>".$logfile) || die "Cannot open log '$logfile'";
print LOG "UPDATE INFO for : NODE17 Seqhound db2 EEE \n";
...
if( $prevfile = glob($fulldir."/".$project."????????.txt*") )
{
...
else
{
	...
	if( $prevfile = glob($fulldir."/".$project."????????_full.txt*") )
	{
		$prevfile =~ /(\d{4})(\d{2})(\d{2})/;
		$last_year = $1;
		$last_month = $2;
	...
	else
	{
		...
		if( $defaultfromdate =~ /(\d{4})(\d{2})(\d{2})/ )
		{
...
$datespec = sprintf("%04d/%02d/%02d:%04d/%02d/%02d", $last_year, $last_month, $last_day, $year, $month, $day);

print "$release\n";
print LOG "SeqHound databases update for $datespec :\n";


# Download HTML update file from NCBI SwissProt repository
system("wget -o $logfile -O $htmlfile -t 3 \"http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Search&db=Protein&term=srcdb+refseq+model+[Properties]%20AND%20${datespec}%20[PDAT]&DISPLAY&doptcmdl=ASN1&dispmax=${maxentries}\"");
if( ! -s $htmlfile )
{
...
if( -s $txtfile.".gz" )
{
	system("rm ${fulldir}/${project}????????.txt*");
	system("mv $txtfile.gz $fulldir");
}
if( -e $project.$today."run" )
{
	system("mv ${project}${today}run $fulllog");
}
if( -e $project.$today."log" )
{
	system("mv ${project}${today}log $fulllog");
}
print LOG "Update succesful\n";
close(LOG);
# Mail entire log to admin
if( $warning )
{
	MailLog();
}
