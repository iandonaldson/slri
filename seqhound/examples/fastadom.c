...
Args myargs[NUMARGS] = {
        ...
        {"minimum domain length","0",NULL,NULL,TRUE,'l',ARG_INT,0.0,0,NULL}
        };
...
Int2 Main()
{
   ...
   for (i = 1; i <= totalrec; i++)  /* fetch each record */
   {
      ...
      if ((set == 0)|| ((Byte) pdd->rep & set)) /* the record belong to a chosen non-redundant set */
       {
            pdom = SHoundMakeFastaDomain(pdd, myargs[0].intvalue,minlen);  /* get FASTA */
            if (pdom == NULL)
             {
               ErrPostEx(SEV_ERROR,0,0, "SHoundMakeFastaDomain failed on mmdbid %ld.", (long) pdd->mmdbid);
...
Revision 1.1.1.4  2002/02/15 09:25:33  root
Automated import from Sourceforge.net

Revision 1.5  2002/02/15 06:42:23  micheld
Modified makefile targets to include dependency building.

Revision 1.4  2002/02/12 21:33:00  kaca
makefile overhaul to accommodate different database layers

Revision 1.3  2002/01/15 16:42:09  kaca
removed unused variable

Revision 1.2  2002/01/15 16:10:58  kaca
